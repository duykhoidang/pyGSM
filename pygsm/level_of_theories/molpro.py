...
class Molpro(Lot):
    ...
    def __init__(self,options):
        ...
        self.file_options.set_active('restarted',False,bool,'')
        ...
        self.occ = self.options['occ']
        self.n_electrons = self.options['n_electrons']
        self.basis = self.options['basis']
        self.n_states = self.options['n_states']
        self.restart = self.options['restarted']
        
        if self.node_id == 0 and not self.restart:
            os.system("cp scratch/starter.wfu scratch/mp_{:04d}_{:04d}".format(self.ID,self.node_id))
    ...
    def run(self,geom):
        ...
        tempfile.write('gthresh,orbital=1.d-7,step=5.d-6\n\n')

        # do singlets
        if len_singlets is not 0:
            tempfile.write(' {multi\n')
            tempfile.write(' start,2140.2\n')
            tempfile.write(' orbital,2140.2\n')
            tempfile.write(' direct\n')
            tempfile.write(' maxiter,21\n')
            ...
            if self.coupling_states:
                tempfile.write('CPMCSCF,NACM,{}.1,{}.1,record=5200.1\n'.format(self.coupling_states[0]+1,self.coupling_states[1]+1))
            tempfile.write(' }\n')
    ...
    def get_gradient(self,coords,multiplicity,state):
        if self.hasRanForCurrentCoords==False or (coords != self.currentCoords).any():
            ...
            geom = manage_xyz.np_to_xyz(self.geom,self.currentCoords)
            self.run(geom)
        tmp = self.search_PES_tuple(self.grada,multiplicity,state)
        print("getting gradient: mult: {} state: {}".format(multiplicity, state))
#        print(self.grada)
#        print(tmp)
        tmp = tmp[0][2]
    ...
    def copy(cls,lot,options,copy_wavefunction=True):
        ...
        if node_id != lot.node_id and copy_wavefunction and not lot.restart:
